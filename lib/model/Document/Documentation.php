...
class Document_Documentation extends Document {
	...
	protected function get_details() {
		...
		$local_details = $db->get_row('SELECT * FROM document_documentation WHERE document_id=?', [ $this->id ]);
	...
	public function save($validate = true) {
		...
		$db = Database::Get();
		$count = $db->get_one('SELECT count(*) FROM document_documentation WHERE document_id=?', [ $this->id ]);
		if ($count == 0) {
			$db->insert('document_documentation', $this->local_details);
		} else {
